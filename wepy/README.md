...
并没有。  
## 真机测试
实现的上拉加载在模拟器上跑的很流畅，不存在性能问题。可是，如果是真机的话会出现两个问题。  
1. 如果是安卓机的话（测试华为荣耀8），上拉加载过程中，会出现明显的卡顿，[解决方法](https://www.w3cschool.cn/weixinapp/weixinapp-tips.html)  
2. 如果是苹果机的话（暂时测试iphone5 和 iPhone7），存在这样一个问题，上拉或下拉回弹效果，这个效果会影响上拉的距离。    
这两个问题想了很久，目前不能优雅的解决。  
